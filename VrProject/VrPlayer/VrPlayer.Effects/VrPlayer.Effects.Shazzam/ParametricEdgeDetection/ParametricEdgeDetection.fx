...
float4 main(float2 uv : TEXCOORD) : COLOR
{
    ...
    float4 c00 = tex2D(inputSampler, texCoord - offX);
    float4 c01 = tex2D(inputSampler, texCoord);
    float4 c02 = tex2D(inputSampler, texCoord + offX);

	// Middle row
	texCoord = uv;
    float4 c10 = tex2D(inputSampler, texCoord - offX);
    float4 c12 = tex2D(inputSampler, texCoord + offX);

	// Bottom row
	texCoord = uv + offY;
    float4 c20 = tex2D(inputSampler, texCoord - offX);
    float4 c21 = tex2D(inputSampler, texCoord);
    float4 c22 = tex2D(inputSampler, texCoord + offX);
